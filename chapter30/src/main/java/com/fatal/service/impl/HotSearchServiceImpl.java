...
public class HotSearchServiceImpl implements IHotSearchService
{
    ...
    {
        Set<Serializable> idSet = zSetOperations.reverseRangeByScore(HotSearchConstant.HOT_SEARCH, 0, Double.MAX_VALUE, 0, HotSearchConstant.TOP_NUMBER);
        if (!CollectionUtils.isEmpty(idSet))
        {
            List<Long> ids = idSet.stream().map(this::toLong).collect(Collectors.toList());
            return ids;
        }
    ...
    {
        Set<ZSetOperations.TypedTuple<Serializable>> typedTuples = zSetOperations.reverseRangeByScoreWithScores(HotSearchConstant.HOT_SEARCH,
                                                                                                                0,
                                                                                                                Double.MAX_VALUE,
                                                                                                                0,
                                                                                                                HotSearchConstant.TOP_NUMBER);
        if (CollectionUtils.isEmpty(typedTuples))
        {
            return new LinkedHashMap<>(16);
        }
        return typedTuples.stream().collect(Collectors.toMap(ZSetOperations.TypedTuple::getValue, ZSetOperations.TypedTuple::getScore,
                                                             // key 重复时执行的方法
