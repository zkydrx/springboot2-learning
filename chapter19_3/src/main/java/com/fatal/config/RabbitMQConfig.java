...
public class RabbitMQConfig
{
    ...
    public RabbitTemplate rabbitTemplate(CachingConnectionFactory connectionFactory)
    {
        ...
        // 设置确认回调函数
        rabbitTemplate.setConfirmCallback((correlationData, ack, cause) -> log.info((ack ? EXIST : NOT_EXIST) + "：time({}), ack({}),cause({})",
        ...
        rabbitTemplate.setReturnCallback((message, replyCode, replyText, exchange, routingKey) -> log.info(
                "消息路由失败：time({}),message({}), replyCode({}), replyText({}), exchange({}), routingKey({})",
                LocalDateTime.now(),
                message,
                replyCode,
                replyText,
    ...
    {
        return BindingBuilder.bind(queue).to(confirmExchange).with(ROUTING_KEY);
    }
